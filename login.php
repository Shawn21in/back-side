...
session_start();
  ...
  if (isset($_POST['usrAccount'])) { //使用者輸入帳號後，帳號密碼的判斷
	 ...
	 $sql = "SELECT * FROM admin where aId='$varAccount'"; // 指定SQL查詢字串
						...
						<div class="login-form">
							...
							<form method="post" action="login.php">
								<div class="form-group">
									<label>帳號</label>
									<input type="text" name = "usrAccount" id="usrAccount" class="form-control" value="<?=$varAccount?>" placeholder="帳號">
								</div>
								<div class="form-group">
									<label>密碼</label>
									<input type="password" name="usrPassword" class="form-control" value="<?=$varPassword?>"  placeholder="密碼">
								</div>
								<div>
									<label>
  										<?=$varErrmessage?>
